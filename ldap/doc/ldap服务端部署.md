...
```bash
echo "192.1t68.127.162    ldap.yileng.club" >> /etc/hosts
```
...
- 编写RootDN的ldif文件（cn=admin,dc=yileng,dc=club）
...
olcAccess: {0}to * by dn.base="gidNumber=0+uidNumber=0,cn=peercred,cn=external,cn=auth"
  read by dn.base="cn=admin,dc=yileng,dc=club" read by * none

dn: olcDatabase={2}hdb,cn=config
changetype: modify
replace: olcSuffix
olcSuffix: dc=yileng,dc=club

dn: olcDatabase={2}hdb,cn=config
changetype: modify
replace: olcRootDN
olcRootDN: cn=admin,dc=yileng,dc=club
...
olcAccess: {0}to attrs=userPassword,shadowLastChange by
  dn="cn=admin,dc=yileng,dc=club" write by anonymous auth by self write by * none
olcAccess: {1}to dn.base="" by * read
olcAccess: {2}to * by dn="cn=admin,dc=yileng,dc=club" write by * read
```
...
dn: cn=Manager,dc=yileng,dc=club
objectClass: organizationalRole
cn: Manager
description: Directory Manager

dn: ou=People,dc=yileng,dc=club
objectClass: organizationalUnit
ou: People

dn: ou=Group,dc=yileng,dc=club
objectClass: organizationalUnit
ou: Group
```
...
```bash
ldapadd -x -D cn=admin,dc=yileng,dc=club -W -f basedomain.ldif    #这里会要求输入openldap数据库的密码，也就是设置的第二个密码
```
...
```bash
ldapsearch -x -b "dc=yileng,dc=club" -H ldap://127.0.0.1
```
